...
namespace SkillLink.Application.Services
{
    public class MisionService : IMisionService
    {
        ...
        public async Task<MisionRespuestaDto> ActualizarProgresoAsync(Guid misionId, Guid usuarioId, int progreso)
        {
            ...
            if (mision.UsuarioAsignadoId != usuarioId)
            {
                throw new UnauthorizedAccessException("No puedes actualizar el progreso de una misión que no tienes asignada.");
            }

            if (mision.Estado == "completada")
            {
                throw new InvalidOperationException("No puedes cambiar el progreso de una misión ya completada.");
            }
        ...
        public async Task<MisionRespuestaDto> ReasignarAsync(Guid misionId, Guid usuarioActualId, Guid nuevoUsuarioId)
        {
            var mision = await _misionRepository.ObtenerPorIdAsync(misionId);
            ...
            {
                throw new UnauthorizedAccessException("No puedes ceder una misión que no tienes asignada.");
            }

            if (mision.Estado == "completada")
            {
                throw new InvalidOperationException("No puedes ceder una misión ya completada.");
            }
